...
def evaluate_predictions(true_file, pred_file, max_deviation=42, main_weight=0.5):
    ...
    if stats['probation_errors']:
        print("示例（前20个）：")
        for case_id, person_name, true_prob, pred_prob in stats['probation_errors'][:10]:
            print(f"  - 案件 {case_id}, 被告人 {person_name}: {true_prob} -> {pred_prob}")
        n1_error = 0
        for case_id, person_name, true_prob, pred_prob in stats['probation_errors']:
            if true_prob*pred_prob == 0:
                n1_error += 1
        print(f"缓刑有无错误: {n1_error}")
...
pred_file = "../output/zui/qwen3-8b-ft-valb-3320_val_xq.json"
