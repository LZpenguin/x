...
API_KEY = "861725f2684f4c2c9886f4ea495eb5f6.q2GyRztXzbAeZeDm"  # 从环境变量获取API密钥

TRAIN_DATA_PATH = '../data/train.json'
N_SHOT = 8
# DATA_PATH = f'../data/val.json'
# OUTPUT_PATH = f'../output/result_v4_n{N_SHOT}.json'
DATA_PATH = f'../data/IntentionalInjury_cases_test.json'
OUTPUT_PATH = f'../output/result.json'
...
try:
    ...
    # 检查输出文件是否存在，如果存在则读取已有结果
    existing_results = {}
    if os.path.exists(OUTPUT_PATH):
        with open(OUTPUT_PATH, 'r', encoding='utf-8') as f:
            existing_results = {case['case_id']: case for case in json.load(f)}
            print(f"找到已有预测结果文件：{OUTPUT_PATH}")
            print(f"已预测案例数：{len([case for case in existing_results.values() if case['case_judgment_label']])}")

    # 过滤出需要预测的案例
    cases_to_predict = []
    for case in test_cases:
        if case['case_id'] in existing_results and existing_results[case['case_id']]['case_judgment_label']:
            # 如果案例已经有预测结果且结果不为空，则使用已有结果
            continue
        cases_to_predict.append(case)

    print(f"需要预测的案例数：{len(cases_to_predict)}")
    results = []
    max_workers = 10
    ...
    if cases_to_predict:
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            # 提交需要预测的任务
            future_to_case = {executor.submit(process_single_case, case): case for case in cases_to_predict}
            
            # 使用tqdm显示进度
            with tqdm(total=len(cases_to_predict), desc="处理案件") as pbar:
    ...
    for case in results:
        cid = case['case_id']
        # 如果还没存，或者当前是成功结果且原来是失败结果，则覆盖
        if (cid not in unique_results or 
            (unique_results[cid]['case_judgment'].startswith("Error") and not case['case_judgment'].startswith("Error"))):
            unique_results[cid] = case

    sorted_results = sorted(unique_results.values(), key=lambda x: x['case_id'])
